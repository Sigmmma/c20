...
# Usage
The recommended document setup for Krita is to have a layer group containing all layers that comprise the texture, then giving that group a transparency mask layer to serve as the alpha channel:
...
4. Right click the group's mask/alpha layer and select _Split Alpha > Save Merged_ to export your `.tif` file. This will export just the contents of the group. Don't forget to set compression to _None_ since [Tool](~h1-tool) does not support deflate compression.
...
Users familiar with Photoshop's _Channels_ window may be confused by Krita's, which shows the _output_ channels of the whole image and are not directly editable.

## Exporting normal maps
Tool generates a normal map when importing a greyscale texture with [_height map_ usage](~/h1/tags/bitmap#tag-field-usage-height-map). Sometimes it's better to generate your normal map in external software and import it as-is, either to have more control or avoid banding on smooth gradients in the height map. Krita can convert a greyscale layer to a normal map using _Filter > Edge Detection > Height to Normal Map_. The XYZ setting you should use is **X+, Y-, Z+**.
...
Import normal maps as tags with [_default_ usage](~h1/tags/bitmap#tag-field-usage) rather than _height map_ to avoid Tool processing them as a height map.