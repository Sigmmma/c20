...
| `+seamsealer` | Used to seal holes at BSP transition areas or in unfinished parts of a level. These faces behave almost the same as `+sky`, except they do not contribute to a cluster being considered an [outdoor cluster](~h1/tags/scenario_structure_bsp#indoor-vs-outdoor-clusters). A cluster with seamsealer faces may still become outdoor if it contains or has [potential visibility](~scenario_structure_bsp#potentially-visible-set) of `+sky` faces.
