...
Maps [downloaded][map-sharing#halonet] by the [Chimera][] mod or compiled by [invader-build][invader] with the `-c` flag use a custom compressed format intended for use by Chimera and takes up less space. The map's header is uncompressed, but all other data is compressed. These files also have a `.map` extension, but are not compatible with the base game. Note that Chimera does not download maps to Halo's main `maps` directory, but take care not to mix these maps with stock ones. When downloaded directly from HaloNet, they have a `.inv` extension.
