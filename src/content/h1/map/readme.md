...
Maps which target H1X should be entirely self contained. This can be accomplished by temporarily removing resource maps from the `maps` directory while compiling the map, since [Tool][] will be unable to find any "cache hits".
...
You can toubleshoot which tags are using the most memory by generating the `baggage.txt` report using the [Sapien][Sapien#game-window] hotkey: <kbd>Control</kbd>+<kbd>Shift</kbd>+<kbd>B</kbd>.
